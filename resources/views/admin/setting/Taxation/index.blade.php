...
@section('header')
    <div class="wrapper wrapper-content animated fadeInRight">
        <div class="col-lg-12">
            <h3><i class="fa fa-list"></i> {{ trans('dashboard.Taxation_Detail') }}</h3>    
            <ol class="breadcrumb">
                <li>
                ...
                <li>
                    ...
                    <a ><span class="nav-label">{{ trans('dashboard.Taxation_Detail_List') }}</span></a>
                </li>
...
@section('content')
    <section class="widget">
        <div class="ibox-title">
                                <h5>{{ trans('dashboard.Taxation_Detail_List') }}</h5>
                                <div class="row"  align="right" style="margin-bottom: 10px;">
                                      <a href="{{ action('Admin\Setting\Taxation_Controller@getCreate') }}"><span class="btn btn-primary btn-xs"> <i class="fa fa-plus"></i> {{ trans('dashboard.Add_Taxation') }}</span></a>
                ...
                <div class="mt">
                    ...
                    <table id="data-table" class="table table-striped table-hover">
                        ...
                        <tr>
                            ...
                            <th class="no-sort hidden-sm-down">{{ trans('dashboard.Tax Form Name') }}</th>
                            <th class="no-sort hidden-sm-down">{{ trans('dashboard.Excies (%)') }}</th>
                            <th class="no-sort hidden-sm-down">{{ trans('dashboard.CST With Form C (%)') }}</th>
                            <th class="no-sort hidden-sm-down">{{ trans('dashboard.CST With Out Form C (%)') }}</th>
                            <th class="no-sort hidden-sm-down">{{ trans('dashboard.VAT (%)') }}</th>
                            <th class="no-sort hidden-sm-down">{{ trans('dashboard.CGST (%)') }}</th>
                            <th class="no-sort hidden-sm-down">{{ trans('dashboard.SGST (%)') }}</th>
                            <th class="no-sort hidden-sm-down">{{ trans('dashboard.IGST (%)') }}</th>
    ...
    <script type="text/javascript">
        ...
        $(function() {
            $dataTable = $('#data-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url : '{!! action("Admin\Setting\Taxation_Controller@getAnydata") !!}',
                    type:'GET',
                },
                aoColumns: [
                    { data: 'taxation_id', name: 'taxation_id', orderable: false, searchable: false},
                    { data: 'tax_name', name: 'tax_name' },
                    { data: 'excies', name: 'excies' },
                    { data: 'cst_with_form_c', name: 'cst_with_form_c' },
                    { data: 'cst_without_form_c', name: 'cst_without_form_c' },
                    { data: 'vat', name: 'vat' },
                    { data: 'cgst', name: 'cgst' },
                    { data: 'sgst', name: 'sgst' },
                    { data: 'igst', name: 'igst' },
...
$(document).on('click','#ac', function(e) {
        ...
        var taxation_id = $(this).attr('data-id');       
        ...
        var self = this;
            if (confirm("Are you sure Status changed!")) {
                ...
                $.ajax({
                    "url": '{!! action("Admin\Setting\Taxation_Controller@anyStatus") !!}',
                    async: false,
                    data: {taxation_id: taxation_id, status: status},
                    method: 'GET',
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 0 : 1;   
            ...
            if (confirm("Are you sure  you want to Active Status!")) {
                $.ajax({
                    "url": '{{ action("Admin\Setting\Taxation_Controller@anyActiveall") }}',
                    async: false,
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 1 : 0;   
            ...
            if (confirm("Are you sure  you want to Inactive Status!")) {
                $.ajax({
                    "url": '{!! action("Admin\Setting\Taxation_Controller@anyInactiveall") !!}',
                    async: false,
                    type: 'GET',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    data: {ids:join_selected_values,status:status},
...
$('.delete_all').on('click', function(e)
     {
        ...
        {  
            ...
            if(check == true){ 
                ...
                $.ajax({
                    "url": '{!! action("Admin\Setting\Taxation_Controller@getRemove")!!}',                        
                    type: 'GET',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
