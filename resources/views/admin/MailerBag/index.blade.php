        ...
        <div class="col-lg-12">
            ...
            <ol class="breadcrumb">
                ...
                <li>
                    ...
                    <a ><span class="nav-label">{{ trans('dashboard.mailer_bag_list') }}</span></a>
                </li>
                    ...
                    <div class="col-lg-12">
                        <div class="ibox float-e-margins">
                            <div class="ibox-title">
                                <h5>Mailer Bag Color Listing</h5>

                                <div class="row"  align="right" style="margin-bottom: 10px;">
                                      <a href="{{ action('Admin\Product\mailer_bag_controller@getCreate') }}"><span class="btn btn-primary btn-xs"> <i class="fa fa-plus"></i> {{ trans('dashboard.new_color') }}</span></a>
                            ...
                            <div class="ibox-content">
                                <div class="table-responsive">
                                    ...
                                    <form id="frmFilter">
                                     ...
                                     <table class="table table-striped table-hover" id="mailerbagTable">
                                        <thead>
                                            <tr>
                                                <th><input type="checkbox" id="select_all"></th>
                                                <th class="no-sort hidden-sm-down">{{ trans('dashboard.mailer_bag_color') }}</th>
...
$('.delete_all').on('click', function(e)
     {
        ...
        {  
            ...
            if(check == true){ 
                ...
                $.ajax({
                    "url": '{!! action("Admin\Product\mailer_bag_controller@getRemove")!!}',                        
                    type: 'GET',
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 0 : 1;   
            ...
            if (confirm("Are you sure Status changed!")) {
                $.ajax({
                    "url": '{{ action("Admin\Product\mailer_bag_controller@anyActiveall") }}',
                    async: false,
...
$('.inactive_all').on('click', function() {
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 1 : 0;   
            ...
            if (confirm("Are you sure Status changed!")) {
                $.ajax({
                    "url": '{{ action("Admin\Product\mailer_bag_controller@anyInactiveall") }}',
                    async: false,
                    data: {ids:join_selected_values,status:status},
                    method: 'GET',
                    success: function (data) {
...
$(function() {
            $dataTable = $('#mailerbagTable').DataTable({
            processing: true,
            serverSide: true,
                ajax: {
                url : '{!! action('Admin\Product\mailer_bag_controller@getData') !!}',
                type:'GET',
            }, 
            aoColumns: [
                { data: 'plastic_color_id', name: 'plastic_color_id',orderable:false, searchable:false },
...
var plastic_color_id = $(this).attr('data-id');       
...
var self = this;
    if (confirm("Are you sure Status changed!")) {
        $.ajax({
            "url": '{!! action("Admin\Product\mailer_bag_controller@anyData1") !!}',
            async: false,
            data: {plastic_color_id: plastic_color_id, status: status},
            method: 'GET',
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            success: function (data) {
                if (data['success']) { 
