        ...
        <div class="col-lg-12">
            ...
            <ol class="breadcrumb">
                ...
                <li>
                    ...
                    <a ><span class="nav-label">{{ trans('dashboard.template_quantity_list') }}</span></a>
                </li>
                    ...
                    <div class="col-lg-12">
                        <div class="ibox float-e-margins">
                            <div class="ibox-title">
                                <h5>{{ trans('dashboard.template_quantity_listing') }}</h5>

                                <div class="row"  align="right" style="margin-bottom: 10px;">
                                      <a href="{{ action('Admin\Template\Template_QuantityController@getCreate') }}"><span class="btn btn-primary btn-xs"> <i class="fa fa-plus"></i> {{ trans('dashboard.new_template_qty') }}</span></a>
                            ...
                            <div class="ibox-content">
                                <div class="table-responsive">
                                    ...
                                    <form id="frmFilter">
                                     ...
                                     <table class="table table-striped table-hover" id="quantityTable">
                                        <thead>
                                            <tr>
...
$(document).on('click','#ac', function(e) {
        ...
        var template_quantity_id = $(this).attr('data-id');       
        ...
        var self = this;
            if (confirm("Are you sure Status changed!")) {
                $.ajax({
                    "url": '{!! action("Admin\Template\Template_QuantityController@anyStatus") !!}',
                    async: false,
                    data: {template_quantity_id: template_quantity_id, status: status},
                    method: 'GET',
  ...
  $('.delete_all').on('click', function(e)
     {
        ...
        {  
            ...
            if(check == true){ 
                ...
                $.ajax({
                    "url": '{!! action("Admin\Template\Template_QuantityController@getRemove")!!}',                        
                    type: 'GET',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
...
$('.active_all').on('click', function() {
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 0 : 1;   
            ...
            if (confirm("Are you sure Status changed!")) {
                $.ajax({
                    "url": '{{ action("Admin\Template\Template_QuantityController@getActiveall") }}',
                    async: false,
...
$('.inactive_all').on('click', function() {
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 1 : 0;   
            ...
            if (confirm("Are you sure Status changed!")) {
                $.ajax({
                    "url": '{{ action("Admin\Template\Template_QuantityController@getInactiveall") }}',
                    async: false,
                    data: {ids:join_selected_values,status:status},
                    method: 'GET',
                    success: function (data) {
...
//End
$(function() {
            $dataTable = $('#quantityTable').DataTable({
            processing: true,
            serverSide: true,
                ajax: {
                url : '{!! action('Admin\Template\Template_QuantityController@getData') !!}',
                type:'GET',
            }, 
            aoColumns: [
                { data: 'template_quantity_id', name: 'template_quantity_id',orderable: false, searchable: false},
    ...
    $(document).on('change','input.js-switch', function() {
        ...
        if (confirm("Are you sure Status changed!")) {
            $.ajax({
                "url": '{!! action("Admin\Template\Template_QuantityController@anyData1") !!}',
                async: false,
                data: {template_quantity_id: template_quantity_id, status: status},
                method: 'GET',
                success: function (data) {
