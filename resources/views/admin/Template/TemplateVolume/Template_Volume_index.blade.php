        ...
        <div class="col-lg-12">
            ...
            <ol class="breadcrumb">
                ...
                <li>
                    ...
                    <a ><span class="nav-label">{{ trans('dashboard.template_volume_list') }}</span></a>
                </li>
                    ...
                    <div class="col-lg-12">
                        <div class="ibox float-e-margins">
                            <div class="ibox-title">
                                <h5>{{ trans('dashboard.template_volume_listing') }}</h5>

                                <div class="row"  align="right" style="margin-bottom: 10px;">
                                      <a href="{{ action('Admin\Template\Template_VolumeController@getCreate') }}"><span class="btn btn-primary btn-xs"> <i class="fa fa-plus"></i> {{ trans('dashboard.new_template_volume') }}</span></a>
                            ...
                            <div class="ibox-content">
                                <div class="table-responsive">
                                    ...
                                    <form id="frmFilter">
                                     ...
                                     <table class="table table-striped table-hover" id="VolumeTable">
                                        <thead>
                                            <tr>
                                                <th><input type="checkbox" id="select_all"></th>
                                                <th class="no-sort hidden-sm-down">{{ trans('dashboard.volume') }}</th>
...
$(document).on('click','#ac', function(e) {
    ...
    var self = this;
        if (confirm("Are you sure Status changed!")) {
            $.ajax({
                "url": '{!! action("Admin\Template\Template_VolumeController@anyStatus") !!}',
                async: false,
...
$('.delete_all').on('click', function(e)
 {
    ...
    {  
        ...
        if(check == true){ 
            ...
            $.ajax({
                "url": '{!! action("Admin\Template\Template_VolumeController@getRemove")!!}',                        
                type: 'GET',
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
...
$('.active_all').on('click', function() {
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 0 : 1;   
            ...
            if (confirm("Are you sure Status changed!")) {
                $.ajax({
                    "url": '{{ action("Admin\Template\Template_VolumeController@anyActiveall") }}',
                    async: false,
...
$('.inactive_all').on('click', function(e) {
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 1 : 0;   
            ...
            if (confirm("Are you sure Status changed!")) {
                $.ajax({
                    "url": '{!! action("Admin\Template\Template_VolumeController@anyInactiveall") !!}',
                    async: false,
                    type: 'GET',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    data: {ids:join_selected_values,status:status},
...
$(function() {
            ...
            serverSide: true,
                ajax: {
                url : '{!! action('Admin\Template\Template_VolumeController@getData') !!}',
                type:'GET',
            }, 
            aoColumns: [
