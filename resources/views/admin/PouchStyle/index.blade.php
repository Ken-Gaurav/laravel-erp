        ...
        <div class="col-lg-12">
            ...
            <ol class="breadcrumb">
                ...
                <li>
                    ...
                    <a ><span class="nav-label">{{ trans('dashboard.pouch_style_list') }}</span></a>
                </li>
                    ...
                    <div class="col-lg-12">
                        <div class="ibox float-e-margins">
                            <div class="ibox-title">
                                <h5>Pouch Style Listing</h5>

                                <div class="row"  align="right" style="margin-bottom: 10px;">
                                      <a href="{{ action('Admin\Product\pouch_style_controller@getCreate') }}"><span class="btn btn-primary btn-xs"> <i class="fa fa-plus"></i> {{ trans('dashboard.new_pouch_style') }}</span></a>
                                     ...
                                     <table class="table table-striped table-hover" id="styleTable">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="no-sort hidden-sm-down">{{ trans('dashboard.pouch_style') }}</th>
...
$('.delete_all').on('click', function(e)
{
    ...
    {  
        ...
        if(check == true){ 
            ...
            $.ajax({
                "url": '{!! action("Admin\Product\pouch_style_controller@getRemove")!!}',                        
                type: 'GET',
    ...
    {  
     ...
     var status = $(this).is(":checked") ? 0 : 1;   
        ...
        if (confirm("Are you sure Status changed!")) {
            $.ajax({
                "url": '{{ action("Admin\Product\pouch_style_controller@anyActiveall") }}',
                async: false,
...
$('.inactive_all').on('click', function() {
    ...
    {  
     ...
     var status = $(this).is(":checked") ? 1 : 0;   
        ...
        if (confirm("Are you sure Status changed!")) {
            $.ajax({
                "url": '{{ action("Admin\Product\pouch_style_controller@anyInactiveall") }}',
                async: false,
                data: {ids:join_selected_values,status:status},
                method: 'GET',
                success: function (data) {
...
$(function() {
        $dataTable = $('#styleTable').DataTable({
        processing: true,
        serverSide: true,
            ajax: {
            url : '{!! action('Admin\Product\pouch_style_controller@getData') !!}',
            type:'GET',
        }, 
        aoColumns: [
            { data: 'pouch_style_id', name: 'pouch_style_id', orderable: false, searchable: false},
...
var pouch_style_id = $(this).attr('data-id');       
...
var self = this;
    if (confirm("Are you sure Status changed!")) {
        $.ajax({
            "url": '{!! action("Admin\Product\pouch_style_controller@anyStatus") !!}',
            async: false,
            data: {pouch_style_id: pouch_style_id, status: status},
            method: 'GET',
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            success: function (data) {
                if (data['success']) { 
