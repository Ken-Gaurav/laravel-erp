...
@section('header')
    <div class="row">
        <div class="col-lg-12">
            <h3><i class="fa fa-list"></i> {{ trans('dashboard.product_material') }}</h3>
            <ol class="breadcrumb">
                <li>
                ...
                <li>
                    ...
                    <a ><span class="nav-label">{{ trans('dashboard.product_material_list') }}</span></a>
                </li>
                    ...
                    <div class="col-lg-12">
                        <div class="ibox float-e-margins">
                            <div class="ibox-title">
                                <h5>{{ trans('dashboard.product_material_listing') }}</h5>

                                <div class="row"  align="right" style="margin-bottom: 10px;">
                                      <a href="{{ action('Admin\Product\Product_MaterialController@getCreate') }}"><span class="btn btn-primary btn-xs"><i class="fa fa-plus"></i>New Product Material</span></a>
                                     ...
                                     <table class="table table-striped table-hover" id="materialTable">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="no-sort hidden-sm-down">{{ trans('dashboard.material_name') }}</th>
                                                <th class="no-sort hidden-sm-down">{{trans('dashboard.thickness') }}</th> 
                                                <th class="no-sort hidden-sm-down">{{trans('dashboard.gsm') }}</th>
 ...
 $('.delete_all').on('click', function(e) {
    ...
    {  
        ...
        if(check == true){  
                    ...
                    $.ajax({
                        "url": '{!! action("Admin\Product\Product_MaterialController@getRemove") !!}',                        
                        type: 'GET',
 ...
 $('.active_all').on('click', function(e) {
    ...
    {  
        ...
        if (confirm("Are you sure Status changed!")) {
                    $.ajax({
                        "url": '{!! action("Admin\Product\Product_MaterialController@anyActiveall") !!}',
                        async: false,
...
//InActive Multiple
 $('.inactive_all').on('click', function(e) {
    ...
    {  
        ...
        if (confirm("Are you sure Status changed!")) {
                    $.ajax({
                        "url": '{!! action("Admin\Product\Product_MaterialController@anyInactiveall") !!}',
                        async: false,
                        data: {ids:join_selected_values,status:status},
                        method: 'GET',
                        success: function (data) {
...
//End
    ...
    $(function() {
            $dataTable = $('#materialTable').DataTable({
            processing: true,
            serverSide: true,
                ajax: {
                url : '{!! action('Admin\Product\Product_MaterialController@getData') !!}',
                type:'GET',
            }, 
            aoColumns: [
