...
@section('header')
    <div class="row">
        <div class="col-lg-12">
            <h3><i class="fa fa-list"></i> {{ trans('dashboard.roll_quantity') }}</h3>
            <ol class="breadcrumb">
                <li>
                ...
                <li>
                    ...
                    <a ><span class="nav-label">{{ trans('dashboard.roll_quantity_list') }}</span></a>
                </li>
                    ...
                    <div class="col-lg-12">
                        <div class="ibox float-e-margins">
                            <div class="ibox-title">
                                <h5>{{ trans('dashboard.roll_quantity_listing') }}</h5>

                                <div class="row"  align="right" style="margin-bottom: 10px;">
                                      <a href="{{ action('Admin\Product\Roll_QuantityController@getCreate') }}"><span class="btn btn-primary btn-xs"><i class="fa fa-plus"></i>{{ trans('dashboard.add') }}</span></a>
                            ...
                            <div class="ibox-content">
                                <div class="table-responsive">
                                    ...
                                    <form id="frmFilter">
                                     ...
                                     <table class="table table-striped table-hover" id="rollquantitytable">
                                        <thead>
                                            <tr>
                                                <th><input type="checkbox" id="select_all"></th>
                                                <th class="no-sort hidden-sm-down">{{ trans('dashboard.quantity') }}</th>
                                                <th class="no-sort hidden-sm-down">{{ trans('dashboard.quantity_type') }}</th>
                                                <th class="no_sort hidden-sm-down">{{ trans('dashboard.plus_minusquantity') }}</th>
...
$('.delete_all').on('click',function(e)
{
        ...
        {  
            ...
            if(check == true){ 
                ...
                $.ajax({
                    "url": '{!! action("Admin\Product\Roll_QuantityController@getRemove")!!}',                        
                    type: 'GET',
                    ...
                    success: function (data) {
                        ...
                        if (data['success']) {
                            ...
                            $('#rollquantitytable').DataTable().draw();                            
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 0 : 1;   
            ...
            if (confirm("Are you sure Status changed!")) {
                $.ajax({
                    "url": '{{ action("Admin\Product\Roll_QuantityController@anyActiveall") }}',
                    async: false,
...
$('.inactive_all').on('click', function() {
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 1 : 0;   
            ...
            if (confirm("Are you sure Status changed!")) {
                $.ajax({
                    "url": '{{ action("Admin\Product\Roll_QuantityController@anyInactiveall") }}',
                    async: false,
                    data: {ids:join_selected_values,status:status},
                    method: 'GET',
                    success: function (data) {
...
$(function() {
            $dataTable = $('#rollquantitytable').DataTable({
            processing: true,
            serverSide: true,
                ajax: {
                url : '{!! action('Admin\Product\Roll_QuantityController@getData') !!}',
                type:'GET',
            }, 
            aoColumns: [
                { data: 'roll_quantity_id', name: 'roll_quantity_id', orderable: false,searchable: false },
                { data: 'quantity', name: 'quantity' },
                { data: 'quantity_type', name: 'quantity_type'},
                { data: 'plus_minus_quantity', name: 'plus_minusquantity'},
...
var roll_quantity_id = $(this).attr('data-id');       
...
var self = this;
    if (confirm("Are you sure Status changed!")) {
        $.ajax({
            "url": '{!! action("Admin\Product\Roll_QuantityController@anyStatus") !!}',
            async: false,
            data: {roll_quantity_id: roll_quantity_id, status: status},
            method: 'GET',
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            success: function (data) {
                if (data['success']) { 
