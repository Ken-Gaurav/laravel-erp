        ...
        <div class="col-lg-12">
            ...
            <ol class="breadcrumb">
                ...
                <li>
                    ...
                    <a href="{{ action('Admin\Product\Stock_Profit_By_SeaController@getIndex') }}" ><span class="nav-label">{{ trans('dashboard.s_p_a_list') }}</span></a>
                </li>
                <li>
                    <i class="fa fa-list"></i>
                    <span class="nav-label">Stock Profit By Sea Detail</span>
                </li>
        ...
        <div class="col-lg-12">
            <div class="ibox">
                <div class="ibox-title">
                    <h5>Stock Profit By Sea Detail</h5>
                </div>
                <div class="ibox-content">
                    <div class="card-box">
                                                       
                    <form class="form-horizontal blog-form" role="form" method="POST" action="{{ action('Admin\Product\Stock_Profit_By_SeaController@postSave') }}" enctype="multipart/form-data">
                         {!! csrf_field() !!}
               ...
               <div class="addmore">
                        <div class="form-group">
                            {{Form::label('', trans('Product Name'),['class' => 'col-md-4 control-label'])}}
                                ...
                                <div class="col-lg-6">
                                    {{Form::label('product_name',old('product_name', strip_tags(isset($stock_profit) ? $stock_profit->product_name: '')),['class' => 'col-md-4 control-label'])}}
                                </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="form-group">
                                ...
                                <div class="col-lg-12 " id="quantity_{{$product_qty->product_quantity_id}}" value="{{$product_qty->product_quantity_id}}">
                                    ...
                                    <div class="row" id="count_value">

                                     {{Form::hidden('count',$count_value,['class' => 'form-control','required'=>'required','id'=>'count'])}}

                                         {{Form::hidden('product_id[]',old('product_id',isset($stock_profit) ? $stock_profit->product_id: ''),['class' => 'col-sm-2 control-label'])}}
                                        ...
                                        <div class="col-lg-4" id="dropdown_<?php  echo $product_qty->product_quantity_id ?>">
                                         ...
                                         {{ Form::select('size[]',$profit,isset($stock_profit) ? $stock_profit->product_zipper_id: '',  array('class' =>'selectoption_'.$product_qty->product_quantity_id.' form-control sel ','id'=>'size1','placeholder'=>'Select Size'))}} 
...
function GetDynamicTextBox(value) {
    ...
    //alert(value);
          ...
          +'<div class="col-sm-2" id="thickness_'+count+'"><input type="hidden" name="product_id[]" value='+{{$stock_profit->product_id}}+' class="form-control validate[required]">'
          +'</div>' 
          +'<div class="col-sm-4" id="dropdown_'+value+'">'
          +'<input id="qty_id" class="hidden" type="text" value='+{{$product_qty->product_quantity_id}}+' name="">'
          +' {!! Form::select("size[]",$profit,isset($stock_profit) ? '': '',  array("class" => "selectoption form-control","placeholder"=>"Select Size","id"=>"size_'+value+'"))!!}'
          +'<select name="size[]" class="selectoption_'+value+' form-control sel" >'
		   +'<option value="">Select Size</option>' 
			@foreach($appprofit as $appprofit)
			
                        +'<option value="{{$appprofit->size_master_id}}">'
						+'{{$appprofit->height}} X {{$appprofit->width}} X {{$appprofit->gs}} {{$appprofit->volume}}( {{$appprofit->zipper_name}} )'
						+'</option>' 
             @endforeach
		  +'</select>'
		  +'</div>'
