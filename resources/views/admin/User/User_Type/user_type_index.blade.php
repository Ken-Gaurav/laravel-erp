...
@section('header')
    <div class="row">
        <div class="col-lg-12">
            <h2>{{ trans('dashboard.User_Type') }}</h2>
            <ol class="breadcrumb">
                <li>
                ...
                <li>
                    ...
                    <a ><span class="nav-label">{{ trans('dashboard.User_Type_List') }}</span></a>
                </li>
                        ...
                        <div class="ibox float-e-margins">
                            <div class="ibox-title">
                                ...
                                <div class="row"  align="right" style="margin-bottom: 10px;">
                                      <a href="{{ action('Admin\User\user_type_controller@getCreate') }}"><span class="btn btn-primary btn-xs"> <i class="fa fa-plus"></i> {{ trans('dashboard.Add_User_Type') }}</span></a>
                            ...
                            <div class="ibox-content">
                                <div class="table-responsive">
                                    ...
                                    <form id="frmFilter">
                                     ...
                                     <table class="table table-striped table-hover" id="User_Type">
                                        <thead>
                                            <tr>
                                                <th><input type="checkbox" id="select_all"></th>
                                                <th class="no-sort hidden-sm-down">{{ trans('dashboard.user_type_name') }}</th>
...
@section('footer_scripts')
    <script type="text/javascript">
      ...
      $(function() {
        $dataTable = $('#User_Type').DataTable({
        processing: true,
        serverSide: true,
            ajax: {
            url : '{!! action('Admin\User\user_type_controller@getAnydata') !!}',
            type:'GET',
        }, 
        aoColumns: [
...
$(document).on('click','#ac', function(e) {
    ...
    var user_type_id = $(this).attr('data-id');       
    ...
    var self = this;
        if (confirm("Are you sure Status changed!")) {
            ...
            $.ajax({
                "url": '{!! action("Admin\User\user_type_controller@anyStatus") !!}',
                async: false,
                data: {user_type_id: user_type_id, status: status},
                method: 'GET',
...
$('.delete_all').on('click', function(e)
{
    ...
    {  
        ...
        if(check == true){ 
            ...
            $.ajax({
                "url": '{!! action("Admin\User\user_type_controller@getRemove")!!}',                        
                type: 'GET',
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
    ...
    {  
     ...
     var status = $(this).is(":checked") ? 0 : 1;   
        ...
        if (confirm("Are you sure  you want to Active Status!")) {
            $.ajax({
                "url": '{{ action("Admin\User\user_type_controller@anyActiveall") }}',
                async: false,
    ...
    {  
     ...
     var status = $(this).is(":checked") ? 1 : 0;   
        ...
        if (confirm("Are you sure  you want to Inactive Status!")) {
            $.ajax({
                "url": '{!! action("Admin\User\user_type_controller@anyInactiveall") !!}',
                async: false,
                type: 'GET',
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                data: {ids:join_selected_values,status:status},
