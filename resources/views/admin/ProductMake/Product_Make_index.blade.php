...
@section('header')
    <div class="row">
        <div class="col-lg-12">
            <h3><i class="fa fa-list"></i> {{ trans('dashboard.product_make') }}</h3>
            <ol class="breadcrumb">
                <li>
                ...
                <li>
                    ...
                    <a ><span class="nav-label">{{ trans('dashboard.product_make_list') }}</span></a>
                </li>
                    ...
                    <div class="col-lg-12">
                        <div class="ibox float-e-margins">
                            <div class="ibox-title">
                                <h5>{{ trans('dashboard.product_make_listing') }}</h5>

                                <div class="row"  align="right" style="margin-bottom: 10px;">
                                      <a href="{{ action('Admin\Product\Product_MakeController@getCreate') }}"><span class="btn btn-primary btn-xs"><i class="fa fa-plus"></i>{{ trans('dashboard.add') }}</span></a>
                            ...
                            <div class="ibox-content">
                                <div class="table-responsive">
                                    ...
                                    <form id="frmFilter">
                                     ...
                                     <table class="table table-striped table-hover" id="productmaketable">
                                        <thead>
                                            <tr>
                                                <th><input type="checkbox" id="select_all"></th>
                                                <th class="no-sort hidden-sm-down">{{ trans('dashboard.make_name') }}</th>
...
$('.delete_all').on('click', function(e)
     {
        ...
        {  
            ...
            if(check == true){ 
                ...
                $.ajax({
                    "url": '{!! action("Admin\Product\Product_MakeController@getRemove")!!}',                        
                    type: 'GET',
                    ...
                    success: function (data) {
                        ...
                        if (data['success']) {
                            ...
                            $('#productmaketable').DataTable().draw();                            
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 0 : 1;   
            ...
            if (confirm("Are you sure Status changed!")) {
                $.ajax({
                    "url": '{{ action("Admin\Product\Product_MakeController@anyActiveall") }}',
                    async: false,
...
$('.inactive_all').on('click', function() {
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 1 : 0;   
            ...
            if (confirm("Are you sure Status changed!")) {
                $.ajax({
                    "url": '{{ action("Admin\Product\Product_MakeController@anyInactiveall") }}',
                    async: false,
                    data: {ids:join_selected_values,status:status},
                    method: 'GET',
                    success: function (data) {
...
$(function() {
            $dataTable = $('#productmaketable').DataTable({
            processing: true,
            serverSide: true,
                ajax: {
                url : '{!! action('Admin\Product\Product_MakeController@getData') !!}',
                type:'GET',
            }, 
            aoColumns: [
                { data: 'make_id', name: 'make_id',orderable: false, searchable: false },
			...
			//[kinjal] updated on 22-11-2017 wed
            $(document).on('click','#ac', function() {
                ...
                if (confirm("Are you sure Status changed!")) {
                    $.ajax({
                        "url": '{!! action("Admin\Product\Product_MakeController@anyData1") !!}',
                        async: false,
                        data: {make_id: make_id, status: status},
                        method: 'GET',
                        success: function (data) {
							//alert(data.success);
                            if (data.success == 'success') {
