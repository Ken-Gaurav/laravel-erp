...
@section('header')
    <div class="wrapper wrapper-content animated fadeInRight">
        <div class="col-lg-12">
        <h3><i class="fa fa-list"></i> {{ trans('dashboard.Product_Category') }}</h3>    
            <ol class="breadcrumb">
                <li>
                ...
                <li>
                    ...
                    <a ><span class="nav-label">{{ trans('dashboard.Product_Category_List') }}</span></a>
                </li>
    ...
    <section class="widget">
        <div class="ibox-title">
                                ...
                                <div class="row"  align="right" style="margin-bottom: 10px;">
                                      <a href="{{ action('Admin\Production\Product_Category_Controller@getCreate') }}"><span class="btn btn-primary btn-xs"> <i class="fa fa-plus"></i> {{ trans('dashboard.Add_Product_Category') }}</span></a>
    ...
    <script type="text/javascript">
        ...
        $(function() {
            $dataTable = $('#data-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url : '{!! action("Admin\Production\Product_Category_Controller@getAnydata") !!}',
                    type:'GET',
                },
                aoColumns: [
                { data: 'product_category_id', name: 'product_category_id', orderable: false, searchable: false},
...
$(document).on('click','#ac', function(e) {
        ...
        var self = this;
            if (confirm("Are you sure Status changed!")) {
                ...
                $.ajax({
                    "url": '{!! action("Admin\Production\Product_Category_Controller@anyStatus") !!}',
                    async: false,
                    data: {product_category_id: product_category_id, status: status},
                    method: 'GET',
...
$('.delete_all').on('click', function(e)
     {
        ...
        {  
            ...
            if(check == true){ 
                ...
                $.ajax({
                    "url": '{!! action("Admin\Production\Product_Category_Controller@getRemove")!!}',                        
                    type: 'GET',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 0 : 1;   
            ...
            if (confirm("Are you sure  you want to Active Status!")) {
                $.ajax({
                    "url": '{{ action("Admin\Production\Product_Category_Controller@anyActiveall") }}',
                    async: false,
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 1 : 0;   
            ...
            if (confirm("Are you sure  you want to Inactive Status!")) {
                $.ajax({
                    "url": '{!! action("Admin\Production\Product_Category_Controller@anyInactiveall") !!}',
                    async: false,
                    type: 'GET',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    data: {ids:join_selected_values,status:status},
