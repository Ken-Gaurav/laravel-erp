...
@section('header')
    <div class="wrapper wrapper-content animated fadeInRight">
        <div class="col-lg-12">
            <h3><i class="fa fa-list"></i> {{ trans('dashboard.Production_Process') }}</h3>    
            <ol class="breadcrumb">
                <li>
                ...
                <li>
                    ...
                    <a ><span class="nav-label">{{ trans('dashboard.Production_Process_List') }}</span></a>
                </li>
...
@section('content')
    <section class="widget">
        <div class="ibox-title">
                                <h5>Production Process List</h5>
                                <div class="row"  align="right" style="margin-bottom: 10px;">
                                      <a href="{{ action('Admin\Production\Production_Process_Controller@getCreate') }}"><span class="btn btn-primary btn-xs"> <i class="fa fa-plus"></i> {{ trans('dashboard.Add_Production_Process') }}</span></a>
                ...
                <div class="mt">
                    ...
                    <table id="data-table" class="table table-striped table-hover">
                        ...
                        <tr>
                            ...
                            <th class="no-sort hidden-sm-down">{{ trans('dashboard.production_process') }}</th>
    ...
    <script type="text/javascript">
        ...
        $(function() {
            $dataTable = $('#data-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url : '{!! action("Admin\Production\Production_Process_Controller@getAnydata") !!}',
                    type:'GET',
                },
                aoColumns: [
                 { data: 'production_process_id', name: 'production_process_id', orderable: false, searchable: false},
                    { data: 'production_process_name', name: 'production_process_name' },
...
$(document).on('click','#ac', function(e) {
        ...
        var production_process_id = $(this).attr('data-id');       
        ...
        var self = this;
            if (confirm("Are you sure Status changed!")) {
                ...
                $.ajax({
                    "url": '{!! action("Admin\Production\Production_Process_Controller@anyStatus") !!}',
                    async: false,
                    data: {production_process_id: production_process_id, status: status},
                    method: 'GET',
...
$('.delete_all').on('click', function(e)
     {
        ...
        {  
            ...
            if(check == true){ 
                ...
                $.ajax({
                    "url": '{!! action("Admin\Production\Production_Process_Controller@getRemove")!!}',                        
                    type: 'GET',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 0 : 1;   
            ...
            if (confirm("Are you sure  you want to Active Status!")) {
                $.ajax({
                    "url": '{{ action("Admin\Production\Production_Process_Controller@anyActiveall") }}',
                    async: false,
        ...
        {  
         ...
         var status = $(this).is(":checked") ? 1 : 0;   
            ...
            if (confirm("Are you sure  you want to Inactive Status!")) {
                $.ajax({
                    "url": '{!! action("Admin\Production\Production_Process_Controller@anyInactiveall") !!}',
                    async: false,
                    type: 'GET',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    data: {ids:join_selected_values,status:status},
