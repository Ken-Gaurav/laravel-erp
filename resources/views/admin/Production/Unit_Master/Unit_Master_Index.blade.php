        ...
        <div class="col-lg-12">
            ...
            <ol class="breadcrumb">
                ...
                <li>
                    ...
                    <a ><span class="nav-label">{{ trans('dashboard.Unit_Master_List') }}</span></a>
                </li>
                        ...
                        <div class="ibox float-e-margins">
                            <div class="ibox-title">
                                ...
                                <div class="row"  align="right" style="margin-bottom: 10px;">
                                      <a href="{{ action('Admin\Production\Unit_Master_Controller@getCreate') }}"><span class="btn btn-primary btn-xs"> <i class="fa fa-plus"></i> {{ trans('dashboard.Add_Unit') }}</span></a>
                                     ...
                                     <table class="table table-striped table-hover" id="unitTable">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="no-sort hidden-sm-down">{{ trans('dashboard.Product_Unit') }}</th>
...
@section('footer_scripts')
    <script type="text/javascript">
      ...
      $(function() {
        ...
        serverSide: true,
            ajax: {
            url : '{!! action('Admin\Production\Unit_Master_Controller@getAnydata') !!}',
            type:'GET',
        }, 
        aoColumns: [
            { data: 'unit_id', name: 'unit_id',orderable: false, searchable: false},
...
$(document).on('click','#ac', function(e) {
    ...
    var self = this;
        if (confirm("Are you sure Status changed!")) {
            ...
            $.ajax({
                "url": '{!! action("Admin\Production\Unit_Master_Controller@anyStatus") !!}',
                async: false,
...
$('.delete_all').on('click', function(e)
{
    ...
    {  
        ...
        if(check == true){ 
            ...
            $.ajax({
                "url": '{!! action("Admin\Production\Unit_Master_Controller@getRemove")!!}',                        
                type: 'GET',
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
    ...
    {  
     ...
     var status = $(this).is(":checked") ? 0 : 1;   
        ...
        if (confirm("Are you sure  you want to Active Status!")) {
            $.ajax({
                "url": '{{ action("Admin\Production\Unit_Master_Controller@anyActiveall") }}',
                async: false,
    ...
    {  
     ...
     var status = $(this).is(":checked") ? 1 : 0;   
        ...
        if (confirm("Are you sure  you want to Inactive Status!")) {
            $.ajax({
                "url": '{!! action("Admin\Production\Unit_Master_Controller@anyInactiveall") !!}',
                async: false,
                type: 'GET',
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                data: {ids:join_selected_values,status:status},
