                ...
                <div class="panel-body">
                   ...
                   <div class="form-group{{ $errors->has('inward_no') ? ' has-error' : '' }}">
                    ...
                    <div class="form-group{{ $errors->has('layers') ? ' has-error' : '' }}">
                        ...
                        {{Form::label('Material', trans('dashboard.Material'),['class' => 'col-md-1 control-label'])}}

                        <div class="test col-md-3" id="material">
                           @for($i=1;$i<=5;$i++)
                            @php $material= app\Models\product_item_info_model::select('product_name','layer_id')->whereRaw('FIND_IN_SET(?,layer_id)',[$i])->get(); @endphp
              


                                  @php $test=[] @endphp
                                @foreach ($material as $material)
                                        @php $test[]=$material->product_name @endphp
                                @endforeach
                                    {!!form::select("material",$test,isset($job) ? $job->material : '',["class"=>"form-control m-b","id"=>"product_material"])!!}
                                @endfor
                ...
                $.ajax({
                    ...
                    success:function(data) {
                        ...
                        for(var j=1;j<=i;j++){
                            ...
                            $.each(data, function(key, value) {
                               $('#product_material').append('<option value="'+ i +'">'+ value +'</option>');
                        });
                            }
