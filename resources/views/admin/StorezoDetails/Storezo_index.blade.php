...
@section('header')
    <div class="row">
        <div class="col-lg-12">
            <h3><i class="fa fa-list"></i> {{ trans('dashboard.storezo') }}</h3>
            <ol class="breadcrumb">
                <li>
                ...
                <li>
                    ...
                    <a ><span class="nav-label">{{ trans('dashboard.storezo_list') }}</span></a>
                </li>
                    ...
                    <div class="col-lg-12">
                        <div class="ibox float-e-margins">
                            <div class="ibox-title">
                                <h5>{{ trans('dashboard.storezo_listing') }}</h5>

                                <div class="row"  align="right" style="margin-bottom: 10px;">
                                      <a href="{{ action('Admin\Product\StorezoDetailController@getCreate') }}"><span class="btn btn-primary btn-xs"><i class="fa fa-plus"></i> New Storezo</span></a>
                            ...
                            <div class="ibox-content">
                                <div class="table-responsive">
                                    ...
                                    <form id="frmFilter">
                                     ...
                                     <table class="table table-striped table-hover" id="storezoTable">
                                        <thead>
                                            <tr>
                                                <th><input type="checkbox" id="select_all" ></th>
                                                <th class="no-sort hidden-sm-down">{{ trans('dashboard.s_name') }}</th>
                                                <th class="no-sort hidden-sm-down">{{trans('dashboard.transport_price') }}</th>
                                                <th class="no-sort hidden-sm-down">{{trans('dashboard.packing_price') }}</th>
 ...
 $('.delete_all').on('click', function(e) {
    ...
    {  
        ...
        if(check == true){  
                    ...
                    $.ajax({
                        "url": '{!! action("Admin\Product\StorezoDetailController@getRemove") !!}',                        
                        type: 'GET',
 ...
 $('.active_all').on('click', function(e) {
    ...
    {  
        ...
        if (confirm("Are you sure Status changed!")) {
                    $.ajax({
                        "url": '{!! action("Admin\Product\StorezoDetailController@anyActiveall") !!}',
                        async: false,
...
//InActive Multiple
 $('.inactive_all').on('click', function(e) {
    ...
    {  
        ...
        if (confirm("Are you sure Status changed!")) {
                    $.ajax({
                        "url": '{!! action("Admin\Product\StorezoDetailController@anyInactiveall") !!}',
                        async: false,
                        data: {ids:join_selected_values,status:status},
                        method: 'GET',
                        success: function (data) {
...
$(function() {
        $dataTable = $('#storezoTable').DataTable({
        processing: true,
        serverSide: true,
            ajax: {
            url : '{!! action('Admin\Product\StorezoDetailController@getData') !!}',
            type:'GET',
        }, 
        aoColumns: [
            { data: 'storezo_id', name: 'storezo_id',orderable: false, searchable: false },
            { data: 'storezo_name', name: 'storezo_name' },
            { data: 'transport_price', name: 'transport_price'},
            { data: 'packing_price', name: 'packing_price'},
...
var storezo_id = $(this).attr('data-id');       
...
var self = this;
    if (confirm("Are you sure Status changed!")) {
        $.ajax({
            "url": '{!! action("Admin\Product\StorezoDetailController@anyStatus") !!}',
            async: false,
            data: {storezo_id: storezo_id, status: status},
            method: 'GET',
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            success: function (data) {
                if (data['success']) { 
